...
## 🔐 نظام المصادقة وصلاحيات المستخدمين

تم تنفيذ نظام كامل للمصادقة وإدارة الصلاحيات. يشمل:
...
2. أنشئ مستخدمين في **Authentication → Users** عبر لوحة Supabase (أو عبر واجهة إدارة المستخدمين في التطبيق — مستقبلاً)
3. أضف سجلات موافقة في جدول `users` مع نفس البريد الإلكتروني وحقل `role`

### إنشاء مستخدم مدير أول (عبر Supabase)
```sql
-- بعد إنشاء المستخدم في Supabase Auth، أضف سجله في جدول users:
INSERT INTO users (doc_data) VALUES (
  '{"name": "مدير النظام", "email": "admin@company.com", "role": "admin", "status": "active"}'
);
```

### أدوار المستخدمين والصلاحيات

| الدور | الوصف | الوحدات المتاحة |
|-------|-------|-----------------|
| `admin` | مدير النظام | جميع الوحدات |
| `accountant` | محاسب | لوحة التحكم، الأصناف، العملاء، الموردين، المشتريات، المبيعات، المصروفات، البنوك، الحسابات، المساعدة |
| `sales` | مندوب مبيعات | لوحة التحكم، الأصناف، العملاء، المبيعات، المخازن، المساعدة |
| `warehouse` | أمين مخزن | لوحة التحكم، الأصناف، المخازن، المساعدة |

### ملفات المصادقة
- `login.html` — صفحة تسجيل الدخول
- `js/auth.js` — منطق المصادقة، جلب الملف الشخصي، تعريف الصلاحيات
