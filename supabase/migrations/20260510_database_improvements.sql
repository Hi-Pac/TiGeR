-- ============================================================
-- TiGeR ERP — Database Architecture Improvements
-- Migration: 20260510_database_improvements.sql
-- Generated: 2026-05-10
--
-- هذا السكريبت يحل جميع مشاكل هندسة قاعدة البيانات المذكورة في التحليل:
-- 1. إضافة Composite Indexes للأداء
-- 2. تحسين قيود المخزون (Inventory Constraints)
-- 3. دوال التحقق من صحة الفواتير (Invoice Validation)
-- 4. مزامنة أرصدة العملاء والموردين (Balance Sync)
-- 5. دوال حركة المخزون الذرية (Atomic Stock Movements)
-- 6. نظام ترقيم الفواتير التلقائي (Invoice Numbering)
-- 7. التحقق من حد الائتمان (Credit Limit)
-- 8. كشف الفواتير المتأخرة (Overdue Detection)
-- 9. جدول المدفوعات (Payments Table)
-- 10. تحسينات RLS للأمان
-- 11. تحسينات Audit Log
-- 12. قيود إضافية للتحقق من صحة البيانات
--
...
DROP INDEX IF EXISTS idx_sales_invoices_company_customer;
CREATE INDEX idx_sales_invoices_company_customer
ON public.sales_invoices(company_id, customer_id);

DROP INDEX IF EXISTS idx_sales_invoices_company_status;
...
DROP INDEX IF EXISTS idx_sales_invoices_overdue;
CREATE INDEX idx_sales_invoices_overdue
...
DROP INDEX IF EXISTS idx_purchase_invoices_company_date;
CREATE INDEX idx_purchase_invoices_company_date
ON public.purchase_invoices(company_id, invoice_date DESC);

DROP INDEX IF EXISTS idx_purchase_invoices_company_supplier;
CREATE INDEX idx_purchase_invoices_company_supplier
ON public.purchase_invoices(company_id, supplier_id);

DROP INDEX IF EXISTS idx_purchase_invoices_company_status;
CREATE INDEX idx_purchase_invoices_company_status
...
DROP INDEX IF EXISTS idx_stock_movements_company_date;
CREATE INDEX idx_stock_movements_company_date
ON public.stock_movements(company_id, movement_date DESC);

DROP INDEX IF EXISTS idx_stock_movements_warehouse_product;
...
CREATE TABLE IF NOT EXISTS public.payments (
    ...
    CONSTRAINT uq_payments_number UNIQUE (company_id, payment_number)
);

CREATE INDEX idx_payments_company ON public.payments(company_id);
CREATE INDEX idx_payments_invoice ON public.payments(invoice_type, invoice_id);
...
-- دالة للتحقق من أن مجموع بنود الفاتورة = المبلغ الإجمالي (Sales)
...
BEGIN
    ...
    -- التحقق من أن subtotal = مجموع البنود
    IF NEW.subtotal_amount != v_items_total THEN
...
-- دالة للتحقق من صحة الفواتير (Purchase)
...
DECLARE
    v_customer_id UUID;
BEGIN
    -- تحديد معرف العميل بناءً على العملية
...
BEGIN
    ...
    WITH customer_balances AS (
        ...
        RETURNING 'customer'::TEXT as entity_type,
                  c.id as entity_id,
                  c.current_balance as old_balance,
                  (SELECT COALESCE(SUM(si.total_amount - si.paid_amount), 0)
                   FROM sales_invoices si
                   WHERE si.customer_id = c.id
                     AND si.invoice_status = 'posted') as new_balance
    )
    SELECT cb.entity_type, cb.entity_id, cb.old_balance, cb.new_balance,
           (cb.new_balance - cb.old_balance) as difference
    ...
    WITH supplier_balances AS (
        ...
        RETURNING 'supplier'::TEXT as entity_type,
                  s.id as entity_id,
                  s.current_balance as old_balance,
                  (SELECT COALESCE(SUM(pi.total_amount - pi.paid_amount), 0)
                   FROM purchase_invoices pi
                   WHERE pi.supplier_id = s.id
                     AND pi.invoice_status = 'posted') as new_balance
    )
    SELECT sb.entity_type, sb.entity_id, sb.old_balance, sb.new_balance,
           (sb.new_balance - sb.old_balance) as difference
...
BEGIN
    ...
    -- تحديث المخزون (سيفشل إذا لم يكن هناك سجل موجود)
...
-- تحديث enum للسماح بحالة 'overdue'
...
-- المستخدم العادي يمكنه تحديث avatar و phone فقط
...
CREATE POLICY pol_profiles_update_self ON public.profiles
    ...
    WITH CHECK (
        id = auth.uid() AND
        -- لا يمكن تغيير الصلاحيات أو الحالة أو الشركة
...
-- منع الحذف الصريح للفواتير (يجب استخدام الإلغاء فقط)
...
-- السماح بالحذف للمسؤولين فقط وفقط للفواتير المسودة
...
-- السماح بالتعديل للمحاسبين والمسؤولين فقط
...
-- View لعرض الفواتير المتأخرة مع تفاصيل العميل
...
-- View محسن لملخص المخزون مع القيم
...
CREATE OR REPLACE FUNCTION fn_calculate_total_inventory_value(
    ...
    p_valuation_method TEXT DEFAULT 'cost'  -- 'cost' or 'selling'
) RETURNS TABLE(
    total_items INTEGER,
...
BEGIN
    ...
    RETURN QUERY
    SELECT
        COUNT(DISTINCT ist.product_id)::INTEGER as total_items,
        SUM(ist.quantity_on_hand)::INTEGER as total_quantity,
        CASE
...
-- تعليقات ختامية
COMMENT ON FUNCTION fn_process_stock_movement IS 'دالة ذرية لمعالجة حركة المخزون مع ضمان التكامل';
COMMENT ON FUNCTION fn_transfer_stock IS 'دالة لنقل المخزون بين المستودعات بشكل آمن';
...
COMMENT ON FUNCTION fn_update_customer_balance IS 'تحديث رصيد العميل تلقائياً عند تغيير الفواتير';
COMMENT ON FUNCTION fn_update_supplier_balance IS 'تحديث رصيد المورد تلقائياً عند تغيير الفواتير';
COMMENT ON FUNCTION fn_reconcile_all_balances IS 'إعادة مزامنة جميع أرصدة العملاء والموردين يدوياً';
COMMENT ON FUNCTION fn_mark_overdue_invoices IS 'تمييز الفواتير المتأخرة تلقائياً';
COMMENT ON FUNCTION fn_calculate_payment_status IS 'حساب حالة الدفع بناءً على المبالغ والتواريخ';
COMMENT ON VIEW v_overdue_invoices IS 'عرض الفواتير المتأخرة مع تفاصيل العملاء';
COMMENT ON VIEW v_payment_summary IS 'ملخص المدفوعات مع تفاصيل الفواتير';
COMMENT ON VIEW v_inventory_valuation IS 'تقييم المخزون بأسعار التكلفة والبيع';
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'الدوال المتاحة للاستخدام:';
    RAISE NOTICE '- fn_process_stock_movement(): معالجة حركة مخزون';
    RAISE NOTICE '- fn_transfer_stock(): نقل مخزون بين مستودعات';
    RAISE NOTICE '- fn_reconcile_all_balances(): مزامنة جميع الأرصدة';
    RAISE NOTICE '- fn_mark_overdue_invoices(): تمييز الفواتير المتأخرة';
    RAISE NOTICE '- fn_calculate_payment_status(): حساب حالة الدفع';
    RAISE NOTICE '- fn_calculate_total_inventory_value(): حساب قيمة المخزون';
    RAISE NOTICE '';
    RAISE NOTICE 'Views الجديدة:';
    RAISE NOTICE '- v_overdue_invoices: الفواتير المتأخرة';
    RAISE NOTICE '- v_payment_summary: ملخص المدفوعات';
    RAISE NOTICE '- v_inventory_valuation: تقييم المخزون';
END $$;
